...
from .distance import cascade_cost, iou_cost_function
from .kalman_utils import chi2inv95


Matches = List[Tuple[int, int]]
MatchingResult = Tuple[Matches, List[int], List[int]]


def matching_cascade(max_age, tracks, dets, track_indices: Union[List[int], None] = None, detection_indices: Union[List[int], None] = None) -> MatchingResult:
    ...
    cost_func = cascade_cost
    cost_threshold = chi2inv95[4]
    ...
    for a in range(1, max_age+1):
        ...
        matches, _, unmatched_det_ids = \
            min_cost_matching(cost_func, cost_threshold, tracks,
                              dets, trk_ids, unmatched_det_ids)
        matched_trks += matches

    unmatched_track_ids = list(
        set(track_indices) - set(k for k, _ in matched_trks))

    return matched_trks, unmatched_track_ids, unmatched_det_ids


def min_cost_matching(distance_metric, max_distance: float, tracks: List[Tracklet], detections: List[Detection], track_indices: List[int], detection_indices: List[int]) -> MatchingResult:

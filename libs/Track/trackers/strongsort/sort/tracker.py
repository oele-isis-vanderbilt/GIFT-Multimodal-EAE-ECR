...
class Tracker:
    ...
    def _initiate_track(self, detection):
        ...
        create = False
        if self.limit_entry:
            map_point = geo.Point(*self.pixel_mapper.detection_to_map(detection.to_xywh(), [0, 1]))
        ...
        if create:    
            self.tracks.append(Track(detection, self._next_id, self.n_init, self.max_age, self.ema_alpha,))
            self._next_id += 1
        #Change End
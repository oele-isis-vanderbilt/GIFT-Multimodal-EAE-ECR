...
def centroid_batch(bboxes1, bboxes2, w=None, h=None) -> np.ndarray:
    """
    ...
    If `w` and `h` are not provided, they are computed from the bounding boxes.
    """
    if w is None or h is None:
        w1, h1 = compute_frame_dimensions(bboxes1)
        w2, h2 = compute_frame_dimensions(bboxes2)
        w = max(w1, w2)
        h = max(h1, h2)
...
def compute_frame_dimensions(bboxes):
    """
    Computes the width and height of the area based on bounding boxes.
    Bounding boxes are in the format [x1, y1, x2, y2].
    """
    x1 = bboxes[..., 0]
    y1 = bboxes[..., 1]
